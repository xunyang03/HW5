...
class DDIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        **kwargs
        ...
        alpha_prod_t = self.alphas_cumprod[timestep]
        alpha_prod_t_prev = self.alphas_cumprod[prev_timestep] if prev_timestep >= 0 else self.final_alpha_cumprod
        # NOTE: alpha_prod_t_prev is alpha bar at t-1 step in the formula
        ...
        elif self.config.clip_sample:   # This will be passed
            pred_original_sample = pred_original_sample.clamp(
                -self.config.clip_sample_range, self.config.clip_sample_range
            )

        # compute variance: "sigma_t(η)"
        variance = self._get_variance(timestep, prev_timestep)
        sigma_t = eta * variance ** (0.5)   

        if use_clipped_model_output:
            raise Exception("This use_clipped_model_output is skipped")


        ############################################ TODO #2: DDIM ###############################################

        # For the following, implement the rest of formula (9), using variable in TODO #1; 
        ...
        if use_DPS:
            ...
            ##################################### TODO #3: Code Starts here for DPS ########################################
            # NOTE: A correct DPS requires a correct DDIM implementation
            # Obtain the variable needed for the calculation
            DPS_scale = 0.5
            ...
            # Implement the Formula (11)
            # NOTE 1: symbol to variable names:
            #           y     -> measurement
